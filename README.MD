...
A kickstarter for creating Android projects at Q42. It gives our projects a flying start and ensures everything we’ve learned from earlier work can be easily reused in new projects.
...
1. Update dependencies and check whether everything still works. After that, create a PR to the
   `main` branch of this template. This helps both your project and this template to stay up to
   date.
...
1. You probably want to enable the self-hosted runner (our own, free and fast, mac-mini), as
   described
...
1. If using Play Store deployment, check the `packageName` in [playstore-internal.yml](.github/workflows/playstore-internal.yml) which should match your package name (the rename-project.py should have done this for you).
1. Build your awesome project :-)
1. Consider contributing to this template when you find something that could be improved.

## Plug and play components

Additional components, that you might or might not want to use, are added as branches.

- [optional/bottom bar navigation](https://github.com/Q42/Template.Android/compare/feature/119/remove-compose-destinations...optional/bottom-bar-navigation)
...
[Handy AI prompts to use with this template](/docs/ai-prompts.md).
...
## How to request new feature's in this template

- Develop it yourself in a branch and create a PR explaining why it's needed in the template. (
  Discussing beforehand with others is recommended)
- Or create a Github issue with the feature request / discussion topic.
...
Copy the file `./dummy_secrets.gradle.kts` to `secrets.gradle.kts` and fill in your own keystore details.

Add the content of secrets.gradle.kts to your Github repository secrets as well, as
`SECRETS_GRADLE`. You can simply copy the content of the file to the github secret, no encoding needed.

Also add the `upload-keystore.jks` file and `secrets.gradle.kts` to 1password. This is important because the secrets cannot be read by any user in github, only by the CI system.
...
Note that the image shows separate modules per feature for domain and data (ie. data:user and
domain:user). It is recommended for larger projects
to make this separation, but for smaller projects, it might be too much overhead. Therefore, this
template starts with a single module for domain and data: domain:main
...
We use Kotlin Script (KTS) for all Gradle build files. This provides type safety,
better IDE support with autocompletion, and compile-time error checking.
...
- `buildSrc/src/main/kotlin/module.feature-and-app.gradle.kts` for all feature modules and the app module
- `buildSrc/src/main/kotlin/module.library.gradle.kts` for all other modules (library modules)

Also, feature-specific gradle files are set up, like `buildSrc/src/main/kotlin/dep.compose.gradle.kts`.
...
wrap your preview content with a `PreviewAppTheme{..}` so that your previews have the correct
theme
background.
...
- Call `InitNavigator(navigator, viewModel)` from navigation setup.
...
We use Kermit because it's usage is close to Timber/Tolbaaken, but Kermit supports KMM.
