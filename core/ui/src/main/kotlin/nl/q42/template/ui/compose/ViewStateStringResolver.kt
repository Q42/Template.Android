 ...
 */
fun ViewStateString.get(context: Context): String {
    return when (this) {
        is ViewStateString.Res -> {
            // Map any nested ViewStateStrings to their resolved values.
            val resolvedArguments =
                this.formatArgs.map { if (it is ViewStateString) it.get(context) else it }
                    .toTypedArray()
            context.resources.getString(this.stringRes, resolvedArguments)
        }
